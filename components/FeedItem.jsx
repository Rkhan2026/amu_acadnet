...
const FeedItem = ({ post }) => {
  return (
    ...
    >
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-center gap-4 text-sm font-bold text-gray-500 mb-2">
            <span className="text-gray-900">{post.authors}</span>
            <span className="w-1 h-1 bg-gray-300 rounded-full" />
            <span className="text-amu-green italic">{post.venue}</span>
          </div>
        </div>
      ...
      <div className="flex items-center justify-end mt-10 pt-8 border-t border-gray-50">
        <button className="flex items-center gap-2 px-8 py-4 bg-amu-green text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl hover:shadow-amu-green/40 hover:-translate-y-1 transition-all">
          Full Paper <BookOpen className="h-4 w-4" />
        </button>
      </div>
    </motion.div>
  );
