...
export async function getCollaboratorRecommendations(universityID) {
  ...
  const currentUserText = `
    Interests: ${currentUser.academicProfile.researchInterests}
  ...
  const recommendations = await Promise.all(
    otherUsers.map(async (user) => {
      const userText = `
        Interests: ${user.academicProfile.researchInterests}
      ...
      const deptBoost = user.department === currentUser.department ? 0.05 : 0;
